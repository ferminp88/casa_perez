...
export function PropuestaValor() {
  ...
  return (
    <section
      ref={ref}
      id="propuesta"
      className="relative bg-paper py-14 md:py-20 overflow-hidden"
    >
      ...
      <div className="container-x relative">
        ...
        <Reveal className="max-w-4xl">
          ...
          <Reveal.Item>
            <h2 className="font-display text-balance text-[clamp(2.25rem,5.5vw,5rem)] font-medium leading-[1.02] tracking-[-0.025em] text-ink">
              Cada evento es{" "}
        ...
        <motion.div
          ...
          className="mt-12 h-px w-full bg-gradient-to-r from-mustard via-ink to-mustard"
        />
        ...
        <div className="mt-12 grid gap-12 md:grid-cols-12 md:gap-14">
          <Reveal className="md:col-span-7">
            <Reveal.Item>
              <p className="mb-8 max-w-xl text-pretty text-base text-ink/75 md:text-lg">
                {BRAND.about}
              </p>
            </Reveal.Item>
            ...
            <ul className="divide-y divide-ink/15 border-t border-ink/15">
              {PILARES.map((p) => (
                <Reveal.Item key={p.n}>
                  <li className="group grid grid-cols-[auto_1fr] items-baseline gap-x-6 gap-y-2 py-6 transition-colors md:grid-cols-[5rem_1fr_auto] md:gap-x-8 md:py-7">
                    <span className="font-display text-3xl font-medium leading-none tracking-tight text-mustard-deep transition-colors group-hover:text-ink md:text-5xl">
                      {p.n}
                    </span>
                    <h3 className="font-display text-lg font-medium leading-snug tracking-tight text-ink md:text-2xl">
                      {p.titulo}
                    </h3>
                    <p className="col-start-2 max-w-xl text-[13.5px] leading-relaxed text-ink/70 md:col-start-2 md:text-[15px]">
                      {p.txt}
                    </p>
                    <span
                      aria-hidden
